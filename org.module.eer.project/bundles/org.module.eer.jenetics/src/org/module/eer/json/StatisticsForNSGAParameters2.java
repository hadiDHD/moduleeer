...
public class StatisticsForNSGAParameters2 {
	...
	public static void main(String[] args) {
		...
		try {
			csvFile = new File("C:\\Users\\hadid\\OneDrive\\Desktop\\University\\05 May\\generations.csv");
			csvFile.createNewFile();
			System.out.println(csvFile.getAbsolutePath());
			csvOut = new FileWriter(csvFile);
			printer = new CSVPrinter(csvOut, CSVFormat.DEFAULT.withDelimiter(',').withHeader("Model Index", "Name",
					"Pareto", "Model Size", "Number of Generations", "Cohesion", "Coupling", "String"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		List<File> jsonFiles = processModuleEERfiles(inputDirectory);
...
//			System.out.println("Progress " + i + " / " + jsonFiles.size());
	...
	private static void loadJson(File file, int modelIndex) {
		...
		if (elements.size() >= 300) {
			...
			for (int g = 100; g <= 2000; g += 100) {
				System.out.println("g = " + g);
				StatisticsModuleEERJeneticsSize nsga = new StatisticsModuleEERJeneticsSize(elements.size(), g);
				List<List<Result>> models = nsga.splitModules2(module, edges);
				writeStats(modelIndex, models, file.getName().replace(".json", ""), elements.size(), g);
			}
		}
	}
	...
	private static void writeStats(int modelIndex, List<List<Result>> models, String modelName, int size,
			int g) {
...
//				System.out.println(results.size());
				...
				for (int j = 0; j < results.size(); j++) {
					...
					printer.printRecord(modelIndex, modelName, j + 1, size, g, result.getCohesion(),
							result.getCoupling(), moduleMap(model));
				}
			}
